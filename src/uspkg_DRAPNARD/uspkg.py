...
from ._metadata import _write_uspkg, _read_uspkg_metadata
from ._utils import _encode_image_to_base64, _verify_file_in_zip

def create_encrypted_uspkg_with_uid(folder_path, output_file, title, description, image_path, update_progress_callback=None):
    ...
    _write_uspkg(output_file, encrypted_zip_data, iv, metadata)
    os.remove(zip_file_path)

def verify_uspkg_file(uspkg_file, uid=None):
    try:
        encrypted_zip_data, iv, metadata = _read_uspkg_metadata(uspkg_file)
